...
| -------- | -------: | --------------: | --------------------: | ------: | ----: | ----------------: | ----------------: | -----------: |
| unopened |        1 |               4 |                 4 / 1 |       0 |    20 |     6.166 / 7.666 |     5.860 / 6.544 |        5,957 |
| visible  |        4 |             304 |             304 / 271 |      12 |     0 | 300.991 / 320.220 | 300.745 / 340.424 |      214,090 |
| loaded   |       11 |          10,004 |                 4 / 1 |      10 |     0 |     4.753 / 4.840 |     5.363 / 5.864 |       14,456 |
| bridges  |        3 |             204 |             204 / 181 |     300 |     0 | 218.453 / 245.405 | 228.910 / 255.754 |      355,491 |
| chain    |        8 |              32 |                32 / 8 |       7 |     0 |   35.851 / 36.441 |   36.163 / 36.558 |       21,615 |
| cycle    |        3 |              12 |                12 / 3 |       3 |     0 |   12.532 / 14.061 |   12.649 / 12.799 |        8,451 |
| diamond  |        4 |              16 |                16 / 4 |       4 |     0 |   17.829 / 18.208 |   17.517 / 18.383 |       11,326 |
| labels   |        3 |              28 |               28 / 13 |       4 |     0 |   20.088 / 21.455 |   19.901 / 20.960 |       22,594 |
...
entries; `local` drops to 4.8 ms because every project but the root participates collapsed, so no
...
- Cold studio startup through the visible composed canvas: **642 ms**. Requests were
...
- Warm local toggle of the toggled target component inside the td frame, 20 measured pairs (40
  samples) after a warm-up pair: input-to-geometry p50 **31.1 ms**, p95 **33.2 ms**. Server
  `layouts.misses` delta **0** and `composed.misses` delta **0** across the measured loop, so no
  unrelated project layout ran.
...
- 50 open/close cycles of the td frame in 13,320 ms, **0** long tasks. Heap after forced GC:
  7,126,535 B before the cycles, 7,627,090 B after the excluded warm-up (cycle 10), 7,850,671 B
...
- Server cache bounds after the run: models 3 entries / 113,939 B, layouts 0 entries / 0 B,
  composed 4 / 209,515 B against the 128 MiB budget; queue 0 active/queued, stale 0, over-limit 0.
...
frames: frame p50 16.7 ms, p99 **19.7 ms**, **0** long tasks over 50 ms, no page errors. This is
...
| Repeated open/close, 50 cycles                 | Caches remain within bounds                             | models 3 entries / 113,939 B, layouts 0 entries / 0 B, composed 4 / 209,515 B against 128 MiB; 0 stale, 0 over-limit                                                                                  | **PASS**                               |
